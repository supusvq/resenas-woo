...
class ReviewImportService:
    ...
    def _upstream_response(self, payload: ImportRequest) -> ImportResponse:
        ...
        job_data = self._wait_for_job(upstream_url, headers, job_id)
        ...
        if not isinstance(raw_reviews, list) or not raw_reviews:
            raise RuntimeError("El scraper upstream no devolvio reseñas.")
    ...
    def _start_upstream_job(self, upstream_url: str, headers: Dict[str, str], payload: ImportRequest) -> str:
        ...
        data = self._json_or_raise(response, "No se pudo crear el trabajo en el scraper upstream.")
    ...
    def _wait_for_job(self, upstream_url: str, headers: Dict[str, str], job_id: str) -> Dict[str, Any]:
        ...
        while time.time() - started_at < timeout_seconds:
            response = requests.get(f"{upstream_url}/jobs/{job_id}", headers=headers, timeout=15)
            data = self._json_or_raise(response, "No se pudo consultar el estado del trabajo upstream.")
    ...
    def _find_place_for_job(self, upstream_url: str, headers: Dict[str, str], maps_url: str) -> Dict[str, Any]:
        response = requests.get(f"{upstream_url}/places", headers=headers, params={"limit": 100}, timeout=20)
        data = self._json_or_raise(response, "No se pudo recuperar la lista de places del scraper upstream.")
    ...
    def _fetch_upstream_reviews(
        ...
        return self._json_or_raise(response, "No se pudieron recuperar las reseñas del scraper upstream.")
